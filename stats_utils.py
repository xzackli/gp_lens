"""
Utility functions for manipulating weak lensing data.

Configure for folders.
"""
...
def get_As(string_with_As):
    """Extract the numerical value of As from a filename."""
...
for row in table:
    ...
    filename_candidates = [x for x in config.folders
                           if np.isclose(row['10^9*A_s'], get_As(x), atol=1e-4)
                           ]

    if '1a(fiducial)' in row['Model']:
        fname = 'Om0.29997_As2.10000_mva0.00000_mvb0.00000'\
            '_mvc0.00000_h0.70000_Ode0.69995'
    elif '1b(fiducial)' in row['Model']:
        fname = 'Om0.29780_As2.10000_mva0.02175_mvb0.02338'\
            '_mvc0.05486_h0.70000_Ode0.69995'
    else:
...
def get_real_list(observable_name, noisy, redshift, smoothing,
                  bin_min, bin_max, binning='050',
                  binscale='', bin_center_row=0):
    """
    Load the realizations from files and format them as arrays.
    ...
    bin_center_row (int) : refers to if we want to use the kappa
                            (use integer 0) or SN (use the integer 1)

    Returns
    -------
        tuple, (array (either ell or kappa), array(mean_stack)), where
            mean_stack is 101 rows, stacked PS or PC
    ...
    for row in table:
        ...
        if redshift == '10_ng40':
            fname = '%s_%s_s%s_z%.2f_ng%s_b%s%s' \
                % (observable_name, noisy, smoothing, 1.0,
                   ng_dict[redshift], binning, binscale)
            ...
            fname = '%s_%s_s%s_z%.2f_ng%s_b%s%s' \
                % (observable_name, noisy, smoothing, float(redshift)/10,
                   ng_dict[redshift], binning, binscale)
            # meandir = 'data/May_stats/%s/Maps%s/%s_Mean.npy' \
            #     % (row['filename'] + modifier, redshift, fname)
            fdir = config.data_folder + '%s/Maps%s/%s.npy' \
    ...
    return bin_centers, mean_stack_list


def get_meanstack(observable_name, noisy, redshift, smoothing,
                  bin_min, bin_max, binning='050', binscale='',
                  bin_center_row=0, start_index=0, end_index=5000):
    """
    Load the weak lensing data and creates arrays of means of realizations.
    ...
    Returns
    -------
        tuple, (array (either ell or kappa), array(mean_stack))
    ...
    for row in table:
        ...
        if redshift == '10_ng40':
            fname = '%s_%s_s%s_z%.2f_ng%s_b%s%s' \
                % (observable_name, noisy, smoothing,
...
def get_meanstack_multiz(observable_name, noisy, redshifts, smoothing,
    ...
    for redshift in redshifts:
        # pass all arguments
        bin_centers, meanstack = get_meanstack(observable_name=observable_name,
                                               noisy=noisy,
                                               redshift=redshift,
                                               smoothing=smoothing,
                                               bin_min=bin_min,
...
def get_invcov(observable_name, noisy, redshifts, smoothing, bin_min, bin_max,
               binning='050', binscale='', bin_center_row=0, sky_coverage=1e4,
               verbose=True):
    """
    Load the fiducial model's covariance.
        ...
        if redshift == '10_ng40':
            fname = '%s_%s_s%s_z%.2f_ng%s_b%s%s' \
                    % (observable_name, noisy, smoothing, 1.0,
                       ng_dict[redshift], binning, binscale)
            fdir = config.data_folder + '%s/Maps%s/%s.npy' \
                % (table[0]['filename'] + modifier, redshift, fname)
        else:
            fname = '%s_%s_s%s_z%.2f_ng%s_b%s%s' \
                % (observable_name, noisy, smoothing, float(redshift)/10,
                   ng_dict[redshift], binning, binscale)
...
#         print(obsarr.shape)
        ...
        # create a function that applies interpolator to the parameters given,
        # for each mode
        self.interp_func = lambda params: \
            np.array([ii(*(params/self.weights)) for ii in spline_interps])

    def P(self, parameter_input):
        """Compute likelihood at a given parameter combination."""
